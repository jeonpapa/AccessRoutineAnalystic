...
## 활성 룰 업데이트 — 2026-07-03 (7차 약평위 D+1 보정)

### 통과 예측 룰 weight 조정

| ID | 이름 | 이전 weight | 신규 weight | 사유 |
|---|---|---|---|---|
| PR-005 | 암질심→약평위 transition | 0.60 | **0.55** | 사이람자 FP (결정신청 트랙 7개월 미진입) |
| PR-009 | NCCN 동등권고 저가포지셔닝 | 0.65 | **0.67** | 테빔브라 6차 TP (weight +0.02) |
| PR-004 | 약평위 재심의 재상정 | 0.70 | **0.72** | 리브리반트 6차 TP (weight +0.02) |

### 신규 CANDIDATE 룰 (2건)

| ID | 이름 | weight | 창출 사유 |
|---|---|---|---|
| PR-NEW-RSA-001 | RSA 확대 사용범위 추적 | 0.72 | 7차 카보메틱스·린파자 RSA 확대 트랙 FN → TP (트랙 분리 학습) |
| PR-NEW-006 | 비oncology 결정신청 scope 확장 | 0.65 | 7차 엑스코프리·스프라바토·옥스루모 FN 연속 4건 — scope 구조 문제 |

### PR-005 개정 요약

**이전**: "암질심 통과 후 3~6개월 약평위 transition (단일 트랙)"  
**이후**: "암질심 통과 후 **결정신청 트랙** 5~9개월 transition. RSA 기존 계약 약제는 PR-NEW-RSA-001 적용"

### 7차 약평위 예측 성과 (2026-07-02)

| 지표 | 값 |
|---|---|
| ON_AGENDA 예측 정확도 (precision) | 1/5 = 0.20 (카보메틱스 RSA 확대 TP, 나머지 FP) |
| ON_AGENDA 포착률 (recall) | 1/8 = 0.13 (결정신청 5건 + RSA 확대 3건 중 카보메틱스 1건만 예측) |
| 주 원인 | ① 결정신청 트랙 비oncology scope 부재 (5개 FN) ② RSA 확대 트랙 미추적 (크리스비타·린파자 FN) |
...
| 2026-07-03 | 7/2 7차 D+1 | **PR-NEW-RSA-001·PR-NEW-006 (2개) + PR-005 개정** | **7차 약평위 FN 분석 — 트랙 혼동·scope 문제 해결** |

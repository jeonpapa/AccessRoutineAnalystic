...
MISSING_SESSIONS = [
    ...
    (2025, 7, 7, "2025-07-10", "YAKPYUNGWI", "HIRA brdBltNo=11531 — 7차 약평위 (씨트렐린·빌베이)"),
...
KEY_ISSUES: dict[str, list[str]] = {
    ...
    "버제니오정": [
        ...
        "6/4 6차 약평위 미진입 — 7/2 7차 상정 후보 (HIRA brdBltNo=11814 공식 기록)",
    ],
    "엘라히어주": [
...
# ── (E) 2026-06-04 6차 약평위 통과 — HIRA brdBltNo=11814 (공식, 4매체 독립검증) ──
...
D(brand_kr="리브리반트주", ingredient_inn="amivantamab", manufacturer="한국얀센",
  ...
  listing_type="신규",
  notes="6차 약평위(2026-06-04) 재상정 통과 — 급여 적정성 있음 (HIRA brdBltNo=11814). "
...
D(brand_kr="테빔브라주 100mg", ingredient_inn="tislelizumab",
  ...
  notes="9차 암질심(2025-12-10 brdBltNo=11684) 5개 적응증 급여기준 설정 → "
        "6차 약평위(2026-06-04 HIRA brdBltNo=11814) 급여범위 확대 적정성 있음 (위험분담 확대 트랙). "
  ...
  events=[dict(committee="AMJILSIM", state="APPROVED",
          ...
          dict(committee="YAKPYUNGWI", state="APPROVED",
               ...
               evidence_url="HIRA brdBltNo=11814 (6차 약평위 RSA 확대 통과)")])
D(brand_kr="빌로이주", ingredient_inn="zolbetuximab", manufacturer="한국아스텔라스제약",
  ...
  listing_type="신규",
  notes="8차 암질심(2025-10-29 brdBltNo=11643) 급여기준 설정 → 6차 약평위(2026-06-04 HIRA brdBltNo=11814) 통과. "
...
D(brand_kr="핀테플라", ingredient_inn="fenfluramine", manufacturer="한국유씨비제약",
  ...
  listing_type="신규",
  notes="6차 약평위(2026-06-04) 통과 — 급여 적정성 있음 (HIRA brdBltNo=11814). "
        "희귀질환 별도 협상 트랙 가능.",
...
D(brand_kr="지텍정 75mg", ingredient_inn="건조계피추출물", manufacturer="종근당",
  ...
  listing_type="신규",
  notes="6차 약평위(2026-06-04) 조건부 통과 — 평가가격 이하 수용 시 (HIRA brdBltNo=11814). "
        "종근당 첫 천연물의약품 급여.",
  events=[dict(committee="YAKPYUNGWI", state="APPROVED",
               session_date="2026-06-04", n_th_attempt=1,
...
def _upsert_event(conn: sqlite3.Connection, drug_id: int, ev: dict,
    ...
    if session_id is None and sdate:
        ...
        if session_id is None:
            # 폴링 잡 견고성: 참조 세션 부재 시 전체 중단 대신 해당 이벤트만 skip(로그).
            # (정상 프로덕션 DB 엔 기본 회차 세션이 이미 존재 — 주로 신규 차수 누락 시 안전망)
            logger.warning("[reimb_import] 세션 미발견 — 이벤트 skip: %s %s (drug_id=%s)",
                           committee, sdate, drug_id)
            return False
...
#   payload = {schema_version, missing_sessions, session_status_updates,
#              expected_next_session, key_issues, drugs, existing_drug_supplements}
# ──────────────────────────────────────────────────────────────────────────────
...
def build_payload() -> dict:
    """모듈 내 파이썬 authoring 데이터 → JSON 직렬화 가능한 payload dict (마이그레이션용)."""
    return {
        "schema_version": SCHEMA_VERSION,
        "missing_sessions": [list(s) for s in MISSING_SESSIONS],
        "session_status_updates": [list(s) for s in SESSION_STATUS_UPDATES],
        "expected_next_session": dict(EXPECTED_NEXT_SESSION),
        "key_issues": {k: list(v) for k, v in KEY_ISSUES.items()},
        "drugs": [dict(d) for d in DRUGS],
        "existing_drug_supplements": [dict(s) for s in EXISTING_DRUG_SUPPLEMENTS],
    }


def export_json(path: Path = DATA_PATH) -> dict:
    """현재 파이썬 데이터를 JSON 파일로 export (1회 마이그레이션 + 재현)."""
    payload = build_payload()
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(_json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return {"path": str(path), "drugs": len(payload["drugs"]),
...
def load_payload(source: "str | Path | dict | None" = None) -> dict:
    """위원회 데이터 로드. source:
       - None       → 로컬 DATA_PATH (없으면 파이썬 authoring 데이터로 fallback)
       - dict       → 그대로 사용
       - http(s) URL→ GET (REIMB_DATA_TOKEN 있으면 Bearer)
       - 파일 경로  → 읽기
    """
    ...
    if source is None:
        if DATA_PATH.exists():
            return _json.loads(DATA_PATH.read_text(encoding="utf-8"))
        return build_payload()  # JSON 미생성 시 파이썬 데이터
    src = str(source)
    if src.startswith(("http://", "https://")):
        req = urllib.request.Request(src, headers={"User-Agent": "ma-reimb-sync/1.0"})
        token = os.environ.get("REIMB_DATA_TOKEN")
        if token:
            req.add_header("Authorization", f"Bearer {token}")
        with urllib.request.urlopen(req, timeout=30) as resp:
            return _json.loads(resp.read().decode("utf-8"))
    return _json.loads(Path(src).read_text(encoding="utf-8"))


def run(source: "str | Path | dict | None" = None) -> dict:
    """위원회 데이터를 프로덕션 DB 에 멱등 적재. source 미지정 시 로컬 JSON/파이썬 데이터.

    반환에 applied_hash 포함 — 폴링 잡이 해시게이트(무중복 import)에 사용.
    """
    payload = load_payload(source)
    p_hash = payload_hash(payload)
    missing_sessions = payload.get("missing_sessions", [])
    status_updates = payload.get("session_status_updates", [])
    expected_next = payload.get("expected_next_session", {})
    key_issues = payload.get("key_issues", {})
    drugs = payload.get("drugs", [])
    supplements = payload.get("existing_drug_supplements", [])
    ...
    try:
        ...
        session_map = _ensure_sessions_from(conn, missing_sessions)

        for row in status_updates:
            sdate, status, note = row[0], row[1], row[2]
            conn.execute(
                "UPDATE amjilsim_sessions SET status = ?, note = ? WHERE session_date = ?",
                (status, note, sdate))
        conn.commit()

        n_sessions = conn.execute("SELECT COUNT(*) c FROM amjilsim_sessions").fetchone()["c"]
        date_to_session = {r["session_date"]: r["session_id"] for r in conn.execute(
            "SELECT session_id, session_date FROM amjilsim_sessions")}

        drugs_before = conn.execute("SELECT COUNT(*) c FROM amjilsim_drugs").fetchone()["c"]
        events_added = 0
        drug_ids = set()
        for d in drugs:
            d = dict(d)
            exp_date = expected_next.get(d["brand_kr"])
        ...
        for brand, issues in key_issues.items():
            conn.execute("UPDATE amjilsim_drugs SET key_issues = ? WHERE brand_kr = ?",
                         (_json.dumps(issues, ensure_ascii=False), brand))
        conn.commit()

        # 선존 DB 행 보강 (브랜드 LIKE) — 웰리렉 등
        for sup in supplements:
            row = conn.execute(
                "SELECT drug_id FROM amjilsim_drugs WHERE brand_kr LIKE ?",
                (sup["brand_like"],)).fetchone()
            if not row:
                continue
            did = row["drug_id"]
            for ev in sup.get("events", []):
                if _upsert_event(conn, did, ev, session_map):
                    events_added += 1
            if sup.get("key_issues"):
                conn.execute("UPDATE amjilsim_drugs SET key_issues = ? WHERE drug_id = ?",
                             (_json.dumps(sup["key_issues"], ensure_ascii=False), did))
        conn.commit()

        drugs_after = conn.execute("SELECT COUNT(*) c FROM amjilsim_drugs").fetchone()["c"]
...
def _ensure_sessions_from(conn: sqlite3.Connection, missing: list) -> dict:
    """payload 의 missing_sessions 로 누락 세션 INSERT(멱등). 반환 {(committee,date):id}."""
    for row in missing:
        year, oa, oo, sdate, committee, note = row[0], row[1], row[2], row[3], row[4], row[5]
        exists = conn.execute(
    ...
    conn.commit()
    rows = conn.execute(
        "SELECT session_id, committee_type, session_date FROM amjilsim_sessions").fetchall()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "export":
        ...
        print(_json.dumps(run(src), ensure_ascii=False, indent=2))
